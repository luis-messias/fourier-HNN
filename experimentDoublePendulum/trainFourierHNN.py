...
def train(seed=0, hidden_dim=200, learn_rate=1e-3, total_steps=2000, print_every=200, B_Fourier=torch.eye(2), Forward_Inputs=True, Learn_B_Fourier=True, nonlinearity=torch.tanh, verbose=True):
    ...
    model = FourierHNN(4, hidden_dim, B_Fourier, Learn_B_Fourier, Forward_Inputs, nonlinearity=nonlinearity)
...
if __name__ == "__main__":
    ...
    for forward_inputs in (True, False):
        for Learn_B in (True, False):
            ...
            B_out_dim = 10
            B_gaussian = torch.randn(4, B_out_dim) * basicGaussianScale
            ...
            # Positional
            B_Positional = torch.cat([torch.eye(4), 2*torch.eye(4), 3*torch.eye(4)], dim=1)
