...
class FourierHNN2(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, B_Fourier, Learn_B_Fourier, Forward_Inputs, assume_canonical_coords=True, nonlinearity=torch.tanh):
        ...
        if Forward_Inputs:  
            mlp_input_dim = B_Fourier.shape[1]*2 + input_dim
        else:
            mlp_input_dim = B_Fourier.shape[1]*2 + int(input_dim/2)
    ...
    def forward(self, y):
        if self.forwardInputs:  
            z = torch.column_stack([y, torch.sin(y[:,0:2] @ self.B_Fourier), torch.cos(y[:,0:2] @ self.B_Fourier)])
        else:
            z = torch.column_stack([ torch.sin(y[:,0:2] @ self.B_Fourier), torch.cos(y[:,0:2] @ self.B_Fourier), y[:,2:]])
...
def train(seed=0, hidden_dim=200, learn_rate=1e-3, total_steps=2000, print_every=200, B_Fourier=torch.eye(2), Forward_Inputs=True, Learn_B_Fourier=True, nonlinearity=torch.tanh, verbose=True):
    ...
    model = FourierHNN2(4, hidden_dim, B_Fourier, Learn_B_Fourier, Forward_Inputs, nonlinearity=nonlinearity)
